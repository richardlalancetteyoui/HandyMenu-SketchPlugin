...
class HandyMenu: NSMenu {
        ...
        if !data.items.isEmpty {
            for (index, item) in data.items.enumerated() {
                ...
                case .command(let commandData):
                    if data.autoGrouping,
                        index > 0,
                        case let CollectionItem.command(previousCommand) = data.items[index-1],
                        previousCommand.pluginID != commandData.pluginID {
                        self.addItem(NSMenuItem.separator())
                    }
                    let newMenuItem = NSMenuItem(title: commandData.name,
                                                 action: #selector(runPluginCommand(sender:)),
                                                 keyEquivalent: "")
    ...
    private func makeTitleView(with title: String) -> NSView {
        ...
        if let settingsImage = Bundle.pluginBundle.image(forResource: .settingsIcon) {
